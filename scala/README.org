...
Use Cases: 
 ...
 + _Implict type conversion_. Allows the compiler to implict convert a
   type A to another B where the type B is needed and a type A is
   supplied. Example: convert a integer 3 values tuple to Date object
   wherever is necessary a data object.

 + _Class Extension_ - Convert objects of class to another adapter class
   with extension methods. It is useful to add new functionality to
   code defined in libraries and sources that cannot be modified.
